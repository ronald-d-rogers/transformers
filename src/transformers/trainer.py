...
class Trainer:
    ...
    def __init__(
        ...
        # Check if `**kwargs` are in model.forward
        self.model_accepts_kwargs = any(k.kind == inspect.Parameter.VAR_KEYWORD for k in forward_params.values())

        # Check if the model has explicit setup for loss kwargs,
        # if not, we will default to the `model_accepts_kwargs` value
        if hasattr(model, "accepts_loss_kwargs"):
            self.model_accepts_loss_kwargs = model.accepts_loss_kwargs
        else:
            self.model_accepts_loss_kwargs = self.model_accepts_kwargs
    ...
    def get_sequence_parallel_kwargs(self):
        ...
        if is_deepspeed_ulysses_enabled() and deepspeed_groups._zero_param_parallel_is_initialized():
            ...
            if sp_group is not None:
                return {
                    "ring_attn_implentation": "deepspeed",
                    "sequence_parallel_group": sp_group,
                    "sequence_parallel_size": sp_group.size(),
                    "sequence_parallel_rank": sp_group.rank(),
                }
        return {}
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.model_accepts_kwargs:
            sequence_parallel_kwargs = self.get_sequence_parallel_kwargs()
            inputs = {**inputs, **sequence_parallel_kwargs}
