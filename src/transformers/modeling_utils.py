...
from .integrations.deepspeed import deepspeed_ring_attention
...
class AlgorithmInterface(MutableMapping):
    """
    Dict-like object keeping track of allowed functions for an algorithm (e.g. attention). You can easily add a new
    function with a call to `register()`. If a model needs to locally overwrite an existing algorithm's function, say
    `sdpa`, it needs to declare a new instance of this class inside the `modeling_<model>.py`, and declare it on that
    instance.
    """
...
class AttentionInterface(AlgorithmInterface):
